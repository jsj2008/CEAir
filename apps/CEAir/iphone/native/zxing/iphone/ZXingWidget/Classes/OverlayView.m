  ...
  if( self ) {
    ...
    if (self.cancelEnabled) {
      ...
      if ([self.cancelButtonTitle length] > 0 ) {
        ...
        [cancelButton setTitle:NSLocalizedStringWithDefaultValue(@"OverlayView cancel button title", nil, [NSBundle mainBundle], @"返回", @"返回") forState:UIControlStateNormal];
      }
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
    self.displayedMessage = NSLocalizedStringWithDefaultValue(@"OverlayView displayed message", nil, [NSBundle mainBundle], @"将二维码装入框中进行扫描", @"将二维码装入框中进行扫描");
  }
